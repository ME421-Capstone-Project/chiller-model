...
# ---------------------------------------------------------------------------
# Bug 1: ChillerLayout.create_grid does not validate ages_years length
#
...
def test_cop_array_is_zero_for_inactive_chillers_with_initial_state():
    ...
    steps = list(sim.stream(duration_hours=1.0, time_step_hours=1.0, initial_state=state))
    for step in steps:
        inactive = ~step.active_mask
        if inactive.any():
