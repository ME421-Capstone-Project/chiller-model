...
The package adopts a modular, composition-based architecture organized into four layers: core, component, model, and simulation.
This design follows the principle of composition over inheritance: the central \texttt{SimulationEnvironment} class holds instances of \texttt{ChillerArray}, \texttt{WindVector}, and a pluggable \texttt{BaseInteractionModel} rather than inheriting from any of them~\cite{ham2015dynamic}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[block, fill=orange!25] (env) at (-2.5, 4.9) {SimulationEnvironment};
        \node[block, fill=orange!25] (opt) at (2.5, 4.9) {Optimizer};

        % Component Layer
        \node[layer, fill=green!8] (comp) at (0, 2.5) {};
        \node[anchor=north west, font=\sffamily\small\bfseries] at ([xshift=5pt, yshift=-5pt] comp.north west) {Component};
        \node[block, fill=green!20] (array) at (-4, 2.4) {ChillerArray};
        \node[block, fill=green!20] (wind) at (0, 2.4) {WindVector};
        \node[block, fill=green!20] (imodel) at (4, 2.4) {InteractionModel};

        % Model Layer
        \node[layer, fill=violet!8] (modl) at (0, 0.0) {};
        \node[anchor=north west, font=\sffamily\small\bfseries] at ([xshift=5pt, yshift=-5pt] modl.north west) {Model};
        \node[block, fill=violet!20] (base) at (-2, -0.1) {BaseInteractionModel};
        \node[block, fill=violet!20] (gauss) at (3, -0.1) {GaussianPlumeModel};

        % Core Layer
        \node[layer, fill=red!8] (core) at (0, -2.5) {};
        \node[anchor=north west, font=\sffamily\small\bfseries] at ([xshift=5pt, yshift=-5pt] core.north west) {Core};
        \node[block, fill=red!15] (cfg) at (-3, -2.6) {configs.py};
        \node[block, fill=red!15] (cst) at (0.5, -2.6) {constants.py};
        \node[block, fill=red!15] (ds) at (4, -2.6) {Data Structures};

        % Arrows: Simulation -> Components
        \draw[arr] (env.south) -- ++(0,-0.5) -| (array.north);
        \draw[arr] (env.south) -- ++(0,-0.5) -| (wind.north);
        \draw[arr] (env.south) -- ++(0,-0.5) -| (imodel.north);
        \draw[darr] (opt.west) -- (env.east);

        % Arrows: Components -> Models
        \draw[darr] (imodel.south) -- (base.north);
        \draw[arr] (gauss.west) -- (base.east) node[midway, above, font=\sffamily\scriptsize] {extends};

        % Arrows: Components -> Core
        \draw[darr] (array.south) -- ++(0,-0.5) -| (cfg.north);
        \draw[darr] (wind.south) -- ++(0,-0.7) -| (cst.north);
        \draw[darr] (gauss.south) -- ++(0,-0.5) -| (ds.north);
    \end{tikzpicture}
...
The \texttt{ChillerSpec} class is implemented as a frozen dataclass, and \texttt{ChillerState} as well as \texttt{PerformanceResult} are defined as \texttt{NamedTuple} instances.
...
Input validation is handled by Pydantic models in the core layer, which enforce physically plausible parameter ranges at the boundary of the system.
For example, the \texttt{ChillerConfig} model rejects COP values exceeding 10 (the practical upper bound for vapour compression cycles) and negative sensitivity coefficients.
